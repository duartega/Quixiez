...
import { getCompanyId} from "../../constants/routes.ts";
import Axios from "axios";
import Complete from './RegistrationComplete';
import {finishRegister} from '../../constants/routes'
...
handleIsValidCompany = isValid =>
    this.setState({ isValidCompany: isValid });

handleSubmit () {

let companyUserId = this.props.companyUserReducer.companyUserId;
let companyId = this.state.id;
    Axios.post(finishRegister, {companyUserId, companyId}).then(result => {
      console.log("success")
    }).catch(err=> {
    ...
    if (companyName.length > 1){
      Axios.get(getCompanyId+companyName).then(result => {
          if (companyName === result.data[0].companyName){
            this.setState({id: result.data[0]['id'], Company: companyName, readyToSubmit: true}) }
      }).catch(err=>{
